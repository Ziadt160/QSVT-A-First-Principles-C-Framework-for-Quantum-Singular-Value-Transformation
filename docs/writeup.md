...
- **`qsp-angles`** — a QSP phase-factor (angle) solver in dependency-free C++.
  ...
  **~25–70× faster** running the *same* algorithm. It's the only **embeddable**
...
and packaged: `pip install qsp-angles`, plus a Rust crate and a pure-C example.
...
- **Quantum linear systems** (`Ax = b` via QSVT matrix inversion): state
  **fidelity 1.0** vs the exact solution, condition numbers 4–16.
...
```bash
pip install qsp-angles                      # the embeddable angle solver
