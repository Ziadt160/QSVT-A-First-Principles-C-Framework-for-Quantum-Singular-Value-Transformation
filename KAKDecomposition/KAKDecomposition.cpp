...
std::tuple<Matrix4cd, Matrix4cd, Matrix4cd> KAKDecomposition::solve()
{
    ...
    SelfAdjointEigenSolver<Matrix4d> eigen_solver(M);
    ...
    Vector4d eigenvalues = eigen_solver.eigenvalues();

    Vector4d sqrt_eigenvalues = eigenvalues.cwiseSqrt();

    Matrix4d Am_real = eigenvalues.asDiagonal();

    Matrix4d P = eigen_solver.eigenvectors();

    if (Am_real.determinant() < 0) {
        Am_real(0, 0) *= -1.0;
    }

    Matrix4d O2_real = P;

    Matrix4d O1_real = U_magic * O2_real.transpose().eval() * Am_real.cwiseInverse();
